        ...
        if (this.isSpecial) {
            if (this.hasHumanPassenger() != null) {
                if (this.petMeta.getEngine().getRideType() == RideType.RUNNING) {
                    ...
                    forMot = this.hasHumanPassenger().bg;
                    if (forMot <= 0.0F) {
                        forMot *= 0.25F;
                    }
                    ...
                    if (!this.world.isClientSide) {
                        this.k(0.35F);
                        super.a(sideMot * (float) Config.getInstance().pet().getModifier_petriding(), f2, forMot * (float) Config.getInstance().pet().getModifier_petriding());
                    }
